...
� C�digo del medicamento (entero, de 3 cifras). 
� Precio (real). 
� Stock (entero). 

Se solicita: 
a. Declarar un tipo de dato que contenga la informaci�n del medicamento. 
b. Declarar un vector de estructura de tipo de datos creado en a. 
c. Cargar en un sector de estructuras, los datos referentes a los medicamentos. Funci�n INGRESO. 
d. Consultar el precio seg�n c�digo del medicamento. Funci�n BUSQUEDA_MEDI. 
e. Informar los c�digos de los medicamentos cuyo stock es inferior a 10 unidades. Funci�n INFORME. */
...
int ingreso (struct medicamentos medi[], int limite){
	...
	do{
			if(band==0){
				printf("\nIngrese c�digo del producto(%d) (termina con 0): ",seingreso+1);
				band++;
			}
			else{
				printf("\nC�digo del producto %d ingresado ya se encuentra en los registros, ingrese otro: ", seingreso+1);	
			}
...
void busqueda_medi(struct medicamentos medi[], int cantcargado){
	...
	printf("\nConsulta de precios seg�n c�digo del medicamento!\n");
	
	while(cod!=0){
				
		printf("\nIngrese c�digo del medicamento que desea consultar su precio (0 para salir de la consulta): ");
		cod=validar(100,999,0);
...
//Informar los c�digos de los medicamentos cuyo stock es inferior a 10 unidades. Funci�n INFORME.
